...
const GraphTest: React.FC<GraphTestProps> = ({ data }) => {
  ...
  context.fillText('Information', canvas.width - arrowSize * 2 - 50, canvas.height - marginBottom - arrowSize);
  ...
  data.forEach((item) => {
    ...
    const yIndex = allowedValues.indexOf(item.y);
    const x = marginLeft + (xIndex * chartWidth) / allowedValues.length;
    const y = marginBottom + ((allowedValues.length - yIndex) * chartHeight) / allowedValues.length;
    const color = item.color;

    context.fillStyle = color;
    if (item.x && item.y) {
      context.fillRect(x - 15, y - 35 , 30, 30);
    }
    });
  };
